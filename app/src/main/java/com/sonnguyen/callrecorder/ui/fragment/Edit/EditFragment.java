...
public class EditFragment extends BaseFragment<EditViewModel>
    ...
    private RecordModel oldRecordModel,newRecordModel;
    private OnActionCallbackFragment callbackFragment;
    private ImageView imvBack, imvDone;
//
...
//    }
    ...
    private void loadFromFile() {
        ...
        mLoadSoundFileThread = new Thread() {
            public void run() {
                try {
                    Log.i("aaa", "mSoundFile = " + mSoundFile);
                    ...
                    if (mSoundFile == null) {
                        ...
                        if (components.length < 2) {
                            err = "Sorry, Ringdroid can't edit a file if it doesn't have an extension like .mp3 or .wav";
                        } else {
                            err = "Sorry, Ringdroid is not yet able to edit files of type";
                        }
                    ...
                    Runnable runnable = new Runnable() {
                        public void run() {
                            showFinalAlert(e, "Error reading file");
                        }
                    };
                    mHandler.post(runnable);
                    return;
...
//        Log.i("aaa", "parent dir = "+parentdir);
//
...
//    }
    ...
    @Override
    protected Class<EditViewModel> getClassModel() {
    ...
    private void onSave() {
//        String phoneNumber = oldRecordModel.getPhoneNumber();
//        int status = oldRecordModel.getStatus();
//        String date = oldRecordModel.getDate();
//        int favourite = oldRecordModel.getFavourite();
//        int trimmed = 1;
        String path = oldRecordModel.getPath();
//        String note = oldRecordModel.getNote();
...
//        String newPath = firstPath + "_new." + mFileType;

//        File parentDir = getContext().getExternalFilesDir(Environment.getStorageDirectory().getPath());
//        String recordFile = "Recording_" + phoneNumber + "_" + oldRecordModel.getId() + "_new." +mFileType;
//        String newPath = parentDir + "/" +recordFile;
//        File file = new File(parentDir,recordFile+"");

//        ContentValues values = new ContentValues();
//        values.put(MediaStore.MediaColumns.DATA, path);
//        values.put(MediaStore.Audio.Media.DURATION, duration);
//        Log.i(TAG, "onSave: value duration = "+values.get(MediaStore.Audio.Media.DURATION));

        // Insert it into the database
//        Uri uri = MediaStore.Audio.Media.getContentUriForPath(path);
//        final Uri newUri = getActivity().getContentResolver().insert(uri, values);
//        getActivity().setResult(RESULT_OK, new Intent().setData(newUri));
        ...
        try {
            ...
            StringWriter writer = new StringWriter();
            e.printStackTrace(new PrintWriter(writer));
            Log.e("Ringdroid", "Error: Failed to create " + path);
            Log.e("Ringdroid", writer.toString());
        }

        oldRecordModel.setTrimmed(1);
        mViewModel.updateRecord(oldRecordModel);
    }
    ...
    protected void initViews() {
        ...
        oldRecordModel = (RecordModel) bundle.getSerializable(MainActivity.KEYBUNDLE_DETAILFRAGMENT_EDIT);
        imvBack = findViewById(R.id.imv_back_edit);
        imvDone = findViewById(R.id.imv_done_edit);

        mFilename = oldRecordModel.getPath();
        Log.i("aaa", "mFileName = " + mFilename);
        mSoundFile = null;
        mKeyDown = false;
